...
class GraphStorage:
    ...
    SIZE_NODE_ID = SIZE_UINT + SIZE_UINT + SIZE_UINT
    ...
    def __init__(self, new: bool, db_name: str = 'db') -> None:
        ...
        if new:
            first_id = 1
            self._write_cur_node_addr(NodeAddress(first_id))
    ...
    def _pack_edge(self, fr: NodeId, to: NodeId,
    ...
    def _set_first_edge_from(self, nid: NodeId, edge: EdgeId) -> None:
        ...
        self._write_uint_to_node_ids(Uint(edge), node_id_address + self.SIZE_UINT)

    def _set_first_edge_to(self, nid: NodeId, edge: EdgeId) -> None:
        node_id_address = self._node_id_address(nid)
        self._write_uint_to_node_ids(
            Uint(edge), node_id_address + 2 * self.SIZE_UINT)
    ...
    def _read_cur_edge_id(self) -> EdgeId:
        ans = self._read_uint_from_edge_ids(EdgeIdAddress(0))
        return EdgeId(ans)

    def _read_cur_node_addr(self) -> NodeAddress:
        ans, _ = self._read_uint_from_nodes(NodeAddress(0))
        return NodeAddress(ans)

    def _write_cur_node_addr(self, x: NodeAddress) -> None:
        self._write_uint_to_nodes(Uint(x), NodeAddress(0))
    ...
    def _write_cur_edge_id(self, x: EdgeId) -> None:
        self._write_uint_to_edge_ids(Uint(x), EdgeIdAddress(0))

    def _write_uint_to_nodes(self, x: Uint, cur: NodeAddress) -> NodeAddress:
        m = self._nodes_mmap
    ...
    def _write_uint_to_edge_ids(self, x: Uint,
                                cur: EdgeIdAddress) -> EdgeIdAddress:
        m = self._edge_ids_mmap
        t = EdgeIdAddress(cur + self.SIZE_UINT)
        m[cur:t] = self._pack_uint(x)
        return t

    def _write_node_to_nodes(self, x: Node, cur: NodeAddress) -> NodeAddress:
    ...
    def _write_edge_to_edges(self, fr: NodeId, to: NodeId,
        ...
        if self._read_first_edge_from_node(fr) == 0:
    ...
    def _set_next_edge_from(self, prev: EdgeId, nxt: EdgeId) -> None:
        edge_id_address = EdgeIdAddress(
            prev * 7 * self.SIZE_UINT + 3 * self.SIZE_UINT)
        self._write_uint_to_edge_ids(Uint(nxt), edge_id_address)

    def _set_next_edge_to(self, prev: EdgeId, nxt: EdgeId) -> None:
        edge_id_address = EdgeIdAddress(
            prev * 7 * self.SIZE_UINT + 5 * self.SIZE_UINT)
        self._write_uint_to_edge_ids(Uint(nxt), edge_id_address)

    def _set_prev_edge_from(self, prev: EdgeId, nxt: EdgeId) -> None:
        edge_id_address = EdgeIdAddress(
            nxt * 7 * self.SIZE_UINT + 2 * self.SIZE_UINT)
        self._write_uint_to_edge_ids(Uint(prev), edge_id_address)

    def _set_prev_edge_to(self, prev: EdgeId, nxt: EdgeId) -> None:
        edge_id_address = EdgeIdAddress(
            nxt * 7 * self.SIZE_UINT + 4 * self.SIZE_UINT)
        self._write_uint_to_edge_ids(Uint(prev), edge_id_address)
    ...
    def _next_edge_to(self, edge: EdgeId) -> EdgeId:
        ...
        ans = self._read_uint_from_edge_ids(Uint(id_addr + 5 * self.SIZE_UINT))
        return EdgeId(ans)

    def _next_edge_from(self, edge: EdgeId) -> EdgeId:
        id_addr = EdgeIdAddress(edge * 7 * self.SIZE_UINT)
        ans = self._read_uint_from_edge_ids(id_addr + 3 * self.SIZE_UINT)
        return EdgeId(ans)

    def _prev_edge_to(self, edge: EdgeId) -> EdgeId:
        id_addr = EdgeIdAddress(edge * 7 * self.SIZE_UINT)
        ans = self._read_uint_from_edge_ids(id_addr + 4 * self.SIZE_UINT)
        return EdgeId(ans)

    def _prev_edge_from(self, edge: EdgeId) -> EdgeId:
        id_addr = EdgeIdAddress(edge * 7 * self.SIZE_UINT)
        ans = self._read_uint_from_edge_ids(id_addr + 2 * self.SIZE_UINT)
        return EdgeId(ans)

    def _node_id_address(self, nid: NodeId) -> NodeIdAddress:
        return NodeIdAddress(nid * self.SIZE_NODE_ID)

    def _link_node_id_to_node_addr(self, nid: NodeId,
                                   naddr: NodeAddress) -> None:
        node_id_address = self._node_id_address(nid)
        s = self._write_uint_to_node_ids(Uint(naddr), node_id_address)
        s = self._write_uint_to_node_ids(Uint(0), s)
        self._write_uint_to_node_ids(Uint(0), s)

    def create_node(self, node: Node) -> NodeId:
        cur_id = self._read_cur_node_id()
        next_id = NodeId(cur_id + 1)
        cur_addr = self._read_cur_node_addr()
        next_addr = self._write_node_to_nodes(node, cur_addr)
        self._write_cur_node_addr(next_addr)
        self._write_cur_node_id(next_id)

        self._link_node_id_to_node_addr(cur_id, cur_addr)
        return cur_id
    ...
    def _read_uint_from_edge_ids(self, f: EdgeIdAddress) -> Uint:
        m = self._edge_ids_mmap
        t = EdgeIdAddress(f + self.SIZE_UINT)
        return self._unpack_uint(m[f:t])

    def _read_float_from_nodes(self, cur: NodeAddress) -> Tuple[Float, NodeAddress]:
        m = self._nodes_mmap
        t = NodeAddress(cur + self.SIZE_FLOAT)
        return self._unpack_float(m[cur:t]), t

    def _read_uint_from_nodes(self, f: NodeAddress) -> Tuple[Uint, NodeAddress]:
        m = self._nodes_mmap
        t = NodeAddress(f + self.SIZE_UINT)
        return self._unpack_uint(m[f:t]), t

    def _read_int_from_nodes(self, cur: NodeAddress) -> Tuple[Int, NodeAddress]:
        m = self._nodes_mmap
        t = NodeAddress(cur + self.SIZE_INT)
        return self._unpack_int(m[cur:t]), t

    def _read_bool_from_nodes(self, f: NodeAddress) -> Tuple[Bool, NodeAddress]:
        m = self._nodes_mmap
        t = NodeAddress(f + self.SIZE_BOOL)
        return self._unpack_bool(m[f:t]), t

    def _read_text_from_nodes(self, f: NodeAddress) -> Tuple[Text, NodeAddress]:
        length, bytes_f = self._read_uint_from_nodes(f)
        bytes_t = NodeAddress(bytes_f + length)
        m = self._nodes_mmap
        bts = m[bytes_f:bytes_t]
        return self._unpack_text_bytes(bts), bytes_t

    def _node_address(self, nid: NodeId) -> NodeAddress:
    ...
    def _read_value_from_nodes(self, cur: NodeAddress) -> Tuple[Any, NodeAddress]:
        m = self._nodes_mmap
        val_desc, cur = self._read_int_from_nodes(cur)
        if val_desc >= 0:
            t = NodeAddress(cur + val_desc)
            text_bytes = m[cur:t]
            return self._unpack_text_bytes(text_bytes), t
        if val_desc == self.TYPE_BOOL:
            return self._read_bool_from_nodes(cur)
        if val_desc == self.TYPE_INT:
            return self._read_int_from_nodes(cur)
        if val_desc == self.TYPE_UINT:
            return self._read_uint_from_nodes(cur)
        if val_desc == self.TYPE_FLOAT:
            return self._read_float_from_nodes(cur)
        raise Exception('Unknown val_desc ' + str(val_desc))

    def _read_node_from(self, naddr: NodeAddress) -> Node:
        ans: Node = Node({})
        cur: NodeAddress = naddr
        rec_len, cur = self._read_uint_from_nodes(cur)
        ans['is_edge'], cur = self._read_bool_from_nodes(cur)
        ans['node_addr'] = naddr
        ans['rec_len'] = rec_len
        ans['props'] = {}
        num_props, cur = self._read_uint_from_nodes(cur)
        for _ in repeat(None, num_props):
            k, cur = self._read_text_from_nodes(cur)
            v, cur = self._read_value_from_nodes(cur)
            ans['props'][k] = v
        return ans

    def _read_edge_from(self, eaddr: EdgeAddress) -> Edge:
        ans: Edge = Edge({})
        ans['fnid'] = self._read_uint_from_edge_ids(eaddr)
        ans['tnid'] = self._read_uint_from_edge_ids(eaddr + self.SIZE_UINT)
        node: Node = self._read_node_from(
            NodeAddress(self._read_uint_from_edge_ids(eaddr + 6 * self.SIZE_UINT)))
    ...
    def get_node(self, nid: NodeId) -> Node:
        ...
        if node_addr == 0:
            return None
        return self._read_node_from(node_addr)

    def get_edge(self, eid: EdgeId) -> Edge:
        """
        Get edge by ID
        """
        if self._read_uint_from_edge_ids(eid * 7 * self.SIZE_UINT) == 0:
            return None
        return self._read_edge_from(eid * 7 * self.SIZE_UINT)

    def _write_packed_node_to_nodes(self, packed: bytes,
    ...
    def update_node(self, nid: NodeId, n: Node) -> None:
        ...
        node = self._read_node_from(old_addr)
        if node is None:
        ...
        if new_rec_len == old_rec_len:
            self._write_packed_node_to_nodes(packed, old_addr)
        else:
            cur_node = self._read_cur_node_addr()
            cur_next_node = self._write_packed_node_to_nodes(packed, cur_node)
    ...
    def delete_node(self, nid: NodeId) -> None:
        ...
        """
        self._link_node_id_to_node_addr(nid, NodeAddress(0))
    ...
    def remove_edge(self, eid: EdgeId) -> None:
        ...
        fr = self._read_uint_from_edge_ids(eaddr)
        to = self._read_uint_from_edge_ids(eaddr + self.SIZE_UINT)
        if self._read_first_edge_from_node(fr) == eid:
        ...
        if self._read_first_edge_to_node(to) == eid:
